...
disk are the only source of truth for _completion_.
...
#### Phase 6 — Markdown-first content pipeline ([milestone #5](https://github.com/evya1/agentic-publishing-pipeline/milestone/5)) _(complete and closed — P6-I04/P6-I05 via PR #96)_
...
#### Phase 7 — Real-source and bibliography pipeline ([milestone #6](https://github.com/evya1/agentic-publishing-pipeline/milestone/6)) _(complete and closed through PR #83)_
...
#### Phase 8 — Python graph generation pipeline ([milestone #7](https://github.com/evya1/agentic-publishing-pipeline/milestone/7)) _(complete and closed through PR #92)_
...
#### Phase 9 — LaTeX project assembly ([milestone #8](https://github.com/evya1/agentic-publishing-pipeline/milestone/8)) _(closed — PR #99)_
...
| P10-I03     | _(planned — GitHub number TBD)_                                       | Calibrate and validate substantive subject-page budget    |
...
| P12-I06     | _(planned — GitHub number TBD)_                                       | Prove genericity through a second-topic integration run                    |
...
| P13-I05     | _(planned — GitHub number TBD)_                                       | Publish a sanitized self-contained final-run evidence bundle |
...
## A. Already completed scaffold work _(verified on disk)_
...
## B. Documentation and planning work _(open)_
...
- [x] Reconcile `docs/PRD_latex_generation.md` with `docs/PRD.md` §8.5:
      ...
      AC §14.3] _Done — commit `4e5517c`._
- [x] Reconcile `docs/PRD_pdf_validation.md` with `docs/PRD.md` FR-40 /
      ...
      NFR-19, AC §14.5] _Done — commit `4e5517c`._
- [x] Reconcile `docs/PRD_bibliography_and_citations.md` with
      ...
      _Done — commit `4e5517c`._
- [x] Refresh `README.md` to point at the canonical Markdown path
...
- [x] Refresh `SUBMISSION_CHECKLIST.md` so its bullets match the PRD
      §14 acceptance criteria one-for-one. [Phase 1] [AC §14.*]
- [x] Decide whether to retire `content/markdown_drafts/` once Phase 6
      begins, or keep it as an alias of `results/generated_markdown/`.
      Record the decision in `docs/PRD_crewai_pipeline.md`. [Phase 1,
      Phase 6] [FR-12]
- [x] Confirm `docs/HW3_REQUIREMENTS.md` still matches the official
      assignment text and capture any deltas in the PRDs. [Phase 1]

## B.5 Phase 1.5 — Demo topic and source manifest lock _(complete — commit `303a425`; downstream follow-ups tracked below and in §C.5)_
...
- [x] Populate `authors:` for each entry in
      ...
      _P7-I02 (issue #23):_ populated for all 10 entries from the
...
- [x] Rekey provisional `tbd…` citation keys to the
      ...
      _P7-I05 (issue #26):_ applied across active artifacts; ledger
...
## C. Future implementation work _(open)_
...
- [x] **P4-I04** — Add C4 system-context, container, and component
      ...
      _Merged, verified, and closed with evidence before Phase 5._

### C.3 Provider/service layer, configuration, and tools _(Phase 5 complete and closed through PR #79)_
...
- [x] Track a reproducible `uv.lock` baseline and codify the per-tool,
      ...
      _Merged through PR #79 — `uv.lock` is tracked and
      `CONTRIBUTING.md §11.5` is the binding rule._
- [x] Implement agent tools under
    ...
    - [x] Search tool for configured-source metadata verification; automatic
          ...
          [FR-5]
          _Merged through PR #79 — `tools/search.py` reads
          `config/article_sources.yaml`, exposes `SearchHit`s through
          the `FixtureSearchAdapter`, and surfaces `verify_arxiv_id` + `manifest_coverage` helpers for the Bibliography Agent._
    - [x] File I/O tool. [Phase 5]
          _Merged through PR #79 — `tools/fileio.FileIO` provides
          atomic writes, path-traversal refusal, and a `fileio.wrote`
          audit event into the run-context event log._
    - [x] Markdown conversion tool (used by the Markdown→LaTeX path).
          [Phase 5, Phase 9]
          _Merged through PR #79 — `tools/markdown.py` provides
    ...
    - [x] LaTeX compilation tool (LuaLaTeX + biber multi-pass). [Phase 5,
          Phase 10] [FR-20]
          _Merged through PR #79 — `tools/latex_build.build_pdf`
          ...
          injected runner is supplied._
    - [x] Graph generation tool. [Phase 5, Phase 8] [FR-29, FR-30]
          _Merged through PR #79 — `visualization/graph.render_line_plot`
          ...
          and emit a JSON provenance file next to each PNG._
- [x] **P5-I08** — Implement versioned Pydantic artifact contracts
      ...
      _Merged through PR #79 — `src/agentic_publishing_pipeline/contracts/`
      ships the E1..E12
...
- [x] **P5-I10** — Introduce `PipelineRunContext`: unique run ID,
      ...
      _Merged through PR #79 — `src/agentic_publishing_pipeline/runtime/`
      ships the workspace,
      manifest, logs, and explicit promotion path._
- [x] **P5-I11** — Add CLI operational modes (`dry-run`,
      ...
      _Merged through PR #79, verified, and closed with evidence._
- [x] **P5-I12** — Implement machine-readable, versioned prompt/config
      ...
      _Merged through PR #79 — registry YAML tree under
      `config/prompt_registry/` + loader and `verify_compatibility`
      in `runtime/registry.py`; pyyaml added to runtime deps._
...
- [x] Record the run in `docs/AI_USAGE.md`. [Phase 6, Phase 13]
      _Merged through PR #81, verified on `main`, and closed with evidence._

### C.5 Real-source and bibliography pipeline

- [x] Define the source-collection policy and the verification procedure.
      [Phase 7] [`docs/PRD_bibliography_and_citations.md` §7]
      _P7-I00 (issue #21):_ policy text fixed in
      ...
      (`docs/TODO.md` §F-6).
- [x] Consume `config/article_sources.yaml` from the Research Agent (T1)
      and the Bibliography Agent (T6) per
      `docs/PRD_crewai_pipeline.md` §6. [Phase 6, Phase 7]
      _P7-I01 (issue #22):_ Bibliography Agent boundary at
      ...
      `docs/TODO.md` §F-6.
- [x] Verify each manifest entry: URL/DOI resolution, title/author/year
      ...
      `docs/TODO.md` §F-6.
- [x] Record the per-source audit trail (citation key → archive →
      ...
      _P7-I03 (issue #24):_ Markdown ledger
      (`docs/SOURCES.md` §5) + machine-readable mirror
      (`results/run_logs/source_verification.json`, schema
      `p7i03-source-audit/v1`) generated by
      `src/agentic_publishing_pipeline/bibliography/audit.py` and
      `run_audit.py`. Includes the three documented placeholder
      corrections per `_audit_corrections.py`. Tests in
      `tests/bibliography/test_audit.py`. Box stays unchecked
      until merge and post-merge evidence per §F-6.
- [x] Extract `.bib` entries from the verified arXiv sources into
      `latex_project/references.bib` (stable keys, no fabricated
      entries). [Phase 7] [FR-19, FR-33]
      _P7-I04 (issue #25):_ `latex_project/references.bib`
      generated by
      `src/agentic_publishing_pipeline/bibliography/bibgen.py` and
      `run_bibgen.py` from the verified manifest only (10
      `@online` entries with `eprinttype=arxiv`, deterministic,
      no fabricated/provisional/rejected). Tests in
      `tests/bibliography/test_bibgen.py` include
      `biber --tool --validate-datamodel` when biber is on PATH.
      Box stays unchecked until merge and post-merge evidence per
      §F-6.
- [x] Rekey provisional `tbd…` citation keys in
      ...
      _P7-I05 (issue #26):_ deterministic migration implemented by
...
- [x] Wire `\cite{...}` placeholders in Markdown drafts and resolve them
      ...
      until merge and post-merge evidence per §F-6.
- [x] Treat all archive contents under
      ...
      _P7-I07 (issue #28):_ enforced by
...
- [x] Replace the placeholder `latex_project/main.tex` with a thin root
      ...
      _`latex_project/main.tex` rewritten: macro definitions precede first
      ...
      `tool_use`; `reasoning` removed). Branch `phase/09-latex-project-assembly`._
...
- [x] Generate chapter files under `chapters/` — narrative text only.
      ...
      `\input{}` calls for tables/figures._
...
- [x] Include at least one substantial Hebrew/English BiDi section with
      ...
      _BiDi sections in `chapters/introduction.tex` (§1.3) and
      `chapters/memory.tex` (§3.3) use `\begin{hebrew}...\end{hebrew}` with
      `\textenglish{}` for inline LTR switches. Pending: verify RTL
      rendering with David CLM under LuaLaTeX._

### C.8 PDF build

- [x] Document and script the multi-pass build (LuaLaTeX → biber →
      LuaLaTeX → LuaLaTeX, plus `makeindex` /
      `makenomenclature` as needed). [Phase 10] [FR-20, §16.3]
- [x] Produce `results/final.pdf` (≥15 pages). [Phase 10] [FR-38, KPI,
      AC §14.2]
- [x] **P10-I03** — Define front/back-matter exclusions, report total
...
## D. Future validation and submission work _(open)_

### D.1 Deterministic ValidatorService

- [x] Implement `src/agentic_publishing_pipeline/validation/` as a
      deterministic `ValidatorService` that runs **after** the Reviewer
      Agent. The LLM must not be the source of truth. [Phase 11]
      [FR-40, NFR-19]
- [x] Validate required repository files. [Phase 11] [FR-34, AC §14.1]
- [x] Validate required LaTeX files (`main.tex`, `preamble.tex`,
      `macros.tex`, `references.bib`, chapters, `tables/`, `figures/`).
      [Phase 11] [FR-35, AC §14.3]
- [x] Validate PDF content indicators where feasible: page count,
...
- [x] Validate every `\cite{...}` resolves to a `references.bib` entry,
      and that bibliography renders. [Phase 11] [FR-33, AC §14.2]
- [x] Produce a human-readable validation report after the run.
      [Phase 11] [FR-37]

### D.2 Tests, lint, reproducibility

- [x] Real unit tests for the provider layer and each tool. [Phase 12]
- [x] Tests for `ValidatorService`. [Phase 12]
- [x] Tests for the Markdown→LaTeX conversion. [Phase 12]
- [x] `uv run pytest` passes on a clean checkout. [Phase 12]
- [x] `uv run ruff check .` passes on a clean checkout. [Phase 12]
- [x] README documents the LaTeX-distribution requirement (LuaLaTeX) and
...
- [x] **P12-I05** — Raise/enforce coverage to at least 85%, harden CI
      ...
      _Merged through PR #86 (issue #85), verified and closed with evidence._
      [Phase 12]
- [x] **P12-I06** — Run the same pipeline with a second topic and a
...
- [x] Update `README.md` to describe the running system: installation,
      configuration, the 8-agent flow, run commands, LaTeX build
      commands, and limitations. [Phase 13] [AC §14.1]
- [x] Fill `docs/AI_USAGE.md` with the real run history (model, prompts
      summary, outputs, human verification, cost). [Phase 13]
- [x] Fill `docs/PROMPTS.md` with the actual prompts used in the run.
      [Phase 13]
- [x] **P13-I05** — Publish a sanitized, self-contained final-run
...
- [x] Verify every checkbox in `SUBMISSION_CHECKLIST.md` is satisfied
      individually. [Phase 14] [AC §14.*]

## E. Cross-cutting policies _(always apply, not phase-gated)_

- [x] Every AI/LLM call is recorded in `docs/AI_USAGE.md` as it happens.
      [Phase 6, Phase 13]
- [x] Every prompt is recorded verbatim in `docs/PROMPTS.md`.
      [Phase 4, Phase 13]
- [x] No secrets committed; `.env` only. [Phase 5] [NFR-20, NFR-21]
- [x] Python files stay ≤150 lines; long files are split by
      responsibility. [NFR-2]
- [x] All functions have type hints including return types. [NFR-7]
- [x] Inputs validated explicitly (e.g., `assert`). [NFR-13]
- [x] DRY and minimal classes/moving parts; extract small named helpers
      instead of long inline blocks. [NFR-5, NFR-6, NFR-12]
- [x] f-strings for interpolation; comprehensions only where they
      improve readability. [NFR-9, NFR-10]

## F. GitHub project management _(Phase 2 — complete and closed; milestone #1 closed)_
